...
#define UTL_VECTOR_INITIALIZER(type) (utl_vector_t) { .array = NULL, .bytes_per_element = sizeof(type) }
#define UTL_VECTOR_DEFAULT(name, type, ...) const type name ##_default[] = {__VA_ARGS__}; utl_vector_t name = { .array = (byte_t*) name ##_default, .size = sizeof(name ##_default) / sizeof(name ##_default[0]), .bytes_per_element = sizeof(type) };
...
static inline utl_vector_t* utl_create_vector(uint32_t bytes_per_element, uint32_t capacity) {
	...
	utl_vector_t init = {
		.array = malloc(capacity * bytes_per_element),
		.size = 0,
		.capacity = capacity,
...
static inline void utl_vector_resize(utl_vector_t* vector, uint32_t new_capacity) {
	...
	if (vector->capacity != 0) {
		vector->array = realloc(vector->array, vector->bytes_per_element * new_capacity);
	} else {
		if (vector->size > 0) {
			void* newarr = malloc(vector->bytes_per_element * new_capacity);
			memcpy(newarr, vector->array, vector->bytes_per_element * vector->size);
			vector->array = newarr;
		} else {
			vector->array = malloc(vector->bytes_per_element * new_capacity);
		}
	}
